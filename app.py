...
/* Accent button styling via markdown */
.accent-text { color: #c8f07a; font-weight: 600; }
...
def call_claude(client, prompt: str, max_tokens: int = 1200) -> str:
...
def analyze_text(client, title: str, text: str) -> dict:
    prompt = f"""Analyze this research paper. Return ONLY a JSON object with:
- summary: 2-3 sentence summary of the paper's contribution
- findings: array of 3-5 key findings (short sentences)
- themes: array of 3-6 research themes (short strings, e.g. "self-attention")
- gaps: array of 1-3 research gaps or limitations (short sentences)
...
Return ONLY valid JSON, no markdown, no preamble."""
    return parse_json_response(call_claude(client, prompt))

def analyze_pdf(client, file_bytes: bytes, filename: str) -> dict:
...
- authors: authors as a string
- year: publication year as string
- journal: journal or venue name
- type: one of "empirical", "methods", "review", "theory"
- abstract: abstract text (or first 300 words)
- summary: 2-3 sentence summary of the paper's contribution
- findings: array of 3-5 key findings (short sentences)
- themes: array of 3-6 research themes (short strings, e.g. "self-attention")
- gaps: array of 1-3 research gaps or limitations (short sentences)
...
Return ONLY valid JSON, no markdown, no preamble."""
    result = parse_json_response(call_claude(client, prompt))
    if not result.get("title"):
        result["title"] = filename.replace(".pdf", "")
    return result

def tag_html(paper_type: str) -> str:
    cls = f"tag-{paper_type}"
    return f'<span class="tag {cls}">{paper_type}</span>'
...
def get_all_gaps():
    ...
    for p in st.session_state.papers:
        for g in p.get("gaps", []):
            gaps.append({"gap": g, "paper": p["title"]})
    return gaps
...
with st.sidebar:
    ...
    if client:
        ...
        st.warning("Enter your API key to begin", icon="🔑")
    ...
    page = st.radio(
        "page",
        ["📚 Library", "📄 Add Paper", "📎 Upload PDF", "🔗 Synthesis", "🔍 Discover"],
...
if page == "📚 Library":
    ...
    if not selected:
            ...
            for i, p in enumerate(st.session_state.papers):
                with cols[i % 2]:
                    ...
                    <div class="paper-card">
                        <h4>{p['title']}</h4>
                        <div class="meta">{p.get('authors','—')} {('· '+p['year']) if p.get('year') else ''}</div>
                        <div style="margin-top:8px">{tag_html(p.get('type','empirical'))}{'<span class="tag tag-pdf">PDF</span>' if p.get('source')=='pdf' else ''}{'<span class="tag tag-review" style="background:rgba(200,240,122,0.08)">analyzed</span>' if p.get('analyzed') else ''}</div>
                    </div>
                    """, unsafe_allow_html=True)
        else:
        ...
        meta_parts = []
        if p.get("authors"): meta_parts.append(p["authors"])
        if p.get("year"): meta_parts.append(p["year"])
        if p.get("journal"): meta_parts.append(p["journal"])
        ...
        with acol1:
            if st.button("✦ Re-analyze", use_container_width=True, disabled=not client):
                if client:
                    with st.spinner("Analyzing..."):
                        result = analyze_text(client, p["title"], p.get("abstract","") + " " + p.get("title",""))
                        p.update({k: result[k] for k in ["summary","findings","themes","gaps"] if k in result})
                        p["analyzed"] = True
                    st.rerun()

        with acol2:
            if st.button("Implications", use_container_width=True, disabled=not client):
                st.session_state[f"action_{p['id']}"] = "implications"
        with acol3:
            if st.button("Methodology", use_container_width=True, disabled=not client):
                st.session_state[f"action_{p['id']}"] = "methods"
        with acol4:
            if st.button("Critical View", use_container_width=True, disabled=not client):
                st.session_state[f"action_{p['id']}"] = "critique"

        action_key = f"action_{p['id']}"
        if action_key in st.session_state and client:
            action = st.session_state[action_key]
            prompts = {
                "implications": f"What are the practical and theoretical implications of this paper? Be specific.\n\nPaper: {p['title']}\n{p.get('abstract','')}",
                "methods": f"Describe the methodology of this paper in detail. Strengths and weaknesses?\n\nPaper: {p['title']}\n{p.get('abstract','')}",
                "critique": f"Provide a critical analysis. Limitations, biases, or challenges to the conclusions?\n\nPaper: {p['title']}\n{p.get('abstract','')}",
            }
            ...
            with st.spinner(f"Generating {label.lower()}..."):
                result = call_claude(client, prompts[action])
...
# ── PAGE: ADD PAPER ───────────────────────────────────────────
    ...
    st.markdown('<div style="color:#888885; font-size:14px; margin-bottom:1.5rem;">Paste the title and abstract to add and analyze a paper.</div>', unsafe_allow_html=True)
    ...
    with st.form("add_paper_form"):
        ...
        submitted = st.form_submit_button("✦ Analyze & Add", type="primary", disabled=not client)
    ...
    if submitted:
        if not title or not abstract:
            ...
            with st.spinner("Analyzing paper..."):
                result = analyze_text(client, title, abstract)
                paper = {
...
# ── PAGE: UPLOAD PDF ──────────────────────────────────────────
    ...
    st.markdown('<div style="color:#888885; font-size:14px; margin-bottom:1.5rem;">Drag and drop PDF files — LitLens will extract the text and analyze them automatically.</div>', unsafe_allow_html=True)

    if not PDF_SUPPORT:
        st.warning("pypdf is not installed. Run: `pip install pypdf` then restart the app.", icon="⚠️")
    elif not client:
        st.warning("Enter your API key in the sidebar first.", icon="🔑")
    else:
        ...
        if uploaded:
            if st.button(f"✦ Analyze {len(uploaded)} PDF{'s' if len(uploaded)>1 else ''}", type="primary"):
                for f in uploaded:
                    with st.spinner(f"Reading and analyzing '{f.name}'..."):
                        try:
                            file_bytes = f.read()
                            result = analyze_pdf(client, file_bytes, f.name)
                            ...
                            paper = {
                                "id": str(datetime.now().timestamp()) + f.name,
                                "title": result.get("title", f.name.replace(".pdf","")),
                                "authors": result.get("authors", ""),
                                "year": result.get("year", ""),
                                "journal": result.get("journal", ""),
                                "type": result.get("type", "empirical"),
...
# ── PAGE: SYNTHESIS ───────────────────────────────────────────
    ...
    if len(papers) < 2:
        ...
        st.markdown(f'<div style="color:#888885; font-size:14px; margin-bottom:1.5rem;">Synthesize across {len(papers)} papers in your library.</div>', unsafe_allow_html=True)
        ...
        if gaps:
            for g in gaps:
                st.markdown(f'<div class="gap-item"><div class="gap-label">gap</div>{g["gap"]}<br><span style="font-size:11px; color:#888885; font-family:DM Mono,monospace;">from: {g["paper"][:60]}{"..." if len(g["paper"])>60 else ""}</span></div>', unsafe_allow_html=True)
        ...
        synth_type = None
        with scol1:
            if st.button("✦ Full Review", use_container_width=True, disabled=not client): synth_type = "full"
        with scol2:
            if st.button("Themes", use_container_width=True, disabled=not client): synth_type = "themes"
        with scol3:
            if st.button("Methods", use_container_width=True, disabled=not client): synth_type = "methods"
        with scol4:
            if st.button("Gaps", use_container_width=True, disabled=not client): synth_type = "gaps"

        if synth_type and client:
            paper_info = "\n\n---\n\n".join([
                f"Title: {p['title']}\nAuthors: {p.get('authors','?')}\nYear: {p.get('year','?')}\nSummary: {p.get('summary', p.get('abstract','')[:300])}\nThemes: {', '.join(p.get('themes',[]))}\nGaps: {'; '.join(p.get('gaps',[]))}"
                for p in papers
            ])
            prompts = {
                "full": f"Write a formal literature review covering these {len(papers)} papers. Include introduction, thematic synthesis (by theme, not paper-by-paper), methodology comparison, and conclusion identifying research gaps. Be scholarly.\n\n{paper_info}",
                "themes": f"Identify and discuss the major cross-cutting themes across these papers. For each theme, cite specific papers.\n\n{paper_info}",
                "methods": f"Compare and contrast the research methodologies across these papers. Discuss strengths and limitations.\n\n{paper_info}",
                "gaps": f"Write a focused section on research gaps, open questions, and future directions based on these papers.\n\n{paper_info}",
            }
            with st.spinner("Generating synthesis..."):
                result = call_claude(client, prompts[synth_type], max_tokens=1500)
...
# ── PAGE: DISCOVER ────────────────────────────────────────────
    ...
    st.markdown('<div style="color:#888885; font-size:14px; margin-bottom:1.5rem;">Describe your research topic and get recommendations for papers, methods, and directions to explore.</div>', unsafe_allow_html=True)

    if not client:
        st.warning("Enter your API key in the sidebar first.", icon="🔑")
    else:
        query = st.text_area("Your research topic", placeholder="e.g. I'm studying transformer attention mechanisms for clinical NLP, focused on interpretability...", height=100)
        ...
        if disc_type and query.strip():
            context = f"\n\nThe researcher currently has these papers: {', '.join(p['title'] for p in st.session_state.papers)}." if st.session_state.papers else ""
            prompts = {
                "general": f"The researcher is studying: '{query}'{context}\n\nProvide: (1) 5-8 key papers to read (with authors and year), (2) 3 recommended databases/tools, (3) key search terms. Be specific.",
                "seminal": f"List the most important seminal/foundational papers for: '{query}'{context}\n\nFor each: title, authors, year, and why it matters.",
                "recent": f"What are the most important recent advances (2020-2025) in: '{query}'{context}\n\nList key papers and describe the trend.",
                "methods": f"What are the best methodological approaches for studying: '{query}'{context}\n\nRecommend specific methods, tools, datasets, and when to use each.",
            }
            with st.spinner("Searching knowledge base..."):
                result = call_claude(client, prompts[disc_type], max_tokens=1200)
            st.markdown('<div class="section-title">Recommendations</div>', unsafe_allow_html=True)
            st.markdown(f'<div class="summary-block" style="white-space:pre-wrap;">{result}</div>', unsafe_allow_html=True)
        elif disc_type and not query.strip():
