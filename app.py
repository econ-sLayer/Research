...
try:
    import pypdfimport streamlit as st
import json
import re
import io
import hashlib
...
# ── Mini save-server for board (runs once in background thread) ─
SAVE_PORT = 8502
def _start_save_server():
    import threading, http.server, json as _j
    class Handler(http.server.BaseHTTPRequestHandler):
        def do_POST(self):
            if self.path == "/board_save":
                try:
                    ...
                    with open(DATA_FILE, "w", encoding="utf-8") as f:
                        _j.dump(raw, f, ensure_ascii=False, indent=2)
    ...
    def run():
        import socketserver
        with socketserver.TCPServer(("", SAVE_PORT), Handler) as httpd:
            httpd.serve_forever()
...
function doSave(){
  ...
  fetch('http://localhost:8502/board_save', {
    method: 'POST',
